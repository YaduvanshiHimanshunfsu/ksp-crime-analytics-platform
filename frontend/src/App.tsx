...
function App() {
  ...
  useEffect(() => { loadDashboard(district, crimeHead).then((bundle) => { setData(bundle); setSelectedAlert(bundle.currentAlerts[0] ?? null); }); }, [district, crimeHead]);
  ...
  return <main className="app-shell">
    ...
    <section className="controlbar"><div className="view-tabs">{(["Command map", "CaseLink", "Risk forecast", "Governance"] as View[]).map((item) => <button key={item} className={view === item ? "active" : ""} onClick={() => setView(item)}>{item}</button>)}</div>
